...
def myChapter(request,pk): # Pk Course id
    ...
    if userid != None:
        ...
        if userv is None:
            ...
            purchaseChapter = Chapter.objects.filter(id__in=Purchase.objects.filter(purchaseuser=userid.user, cours_id=pk ,status=True).values('chapter_id')).values('id','title')
            # allChapter = Chapter.objects.filter(course=pk).values('id', 'title')
            return Response({'purchaseChapter': purchaseChapter}) # ,'allChapter' : allChapter
    else:
        return Response({'status': "please login First"})
...
@csrf_exempt
@api_view(["POST","GET"])
def myChaptervideo(request,pk): # Pk Course id
    t = request.META['HTTP_AUTHORIZATION']
    userid=Tokens(tok=t)
    if userid != None:
        userv=userid.user.id
...
class FileUploadViewSet(ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        serializer.save(owner_id=data.id ,name=self.request.data.get('name'), courses_id=self.request.data.get('courses'), chapter_id=self.request.data.get('chapter'), uploadfile=self.request.data.get('uploadfile'))
